...
int main(){
    ...
    while(!pq.empty()){
        ...
        for(int i =0;i<4;i++){
            if(ni + move[i].first >=0 && ni + move[i].first < r && nj + move[i].second < c  && nj+move[i].second >= 0 ){
                if(w + g[ni + move[i].first][nj + move[i].second] < dist[ni+move[i].first][nj + move[i].second]){
                    ...
                    pq.push({ni+move[i].first,nj + move[i].second,w + g[ni + move[i].first][nj + move[i].second]});
                }
            }
        }
    }
